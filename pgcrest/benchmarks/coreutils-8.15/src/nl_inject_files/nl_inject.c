...
struct linebuffer *
fuzzreadlinebuffer (struct linebuffer *linebuffer, FILE *stream){
    int i,limit,tmp;
    static count = 0;
    char *buffer = linebuffer->buffer;
    int num = rand()%MYLINELEN+1;
...
#ifdef  CREST
  line_buf.buffer = (char*)malloc(sizeof(char)*(MYMAX+1));
  memset(line_buf.buffer,0,sizeof(char)*(MYMAX+1));
  line_buf.size = MYMAX+1;
  line_buf.length = 0;
  CREST_unsigned_int(line_buf.length);
  while (fuzzreadlinebuffer (&line_buf, fp))
#else
