...
Search::Search(const string& program, int max_iterations)
  ...
  /*// Read branch reachability to path-fragment
...
*/
  ...
  for(int i=0;i<branches_.size();i++){
	...
	for(int j=0;j<branch2path_reachable_[branches_[i]].size();j++){
		fprintf(stderr," %d ",(branch2path_reachable_[branches_[i]][j]==true)?1:0);
	}
	fprintf(stderr,"\n");
  }
   */
...
int Search::PathGuidedSolveAtBranch(const SymbolicExecution& ex,
                           size_t branch_idx,
                           vector<value_t>* input) {
  
  const vector<SymbolicPred*>& constraints = ex.path().constraints();

  bool contradic = false;
  bool reachall = true;
  ...
  if(contradic && reachall)		//path mark contradic
	  return -1;
  else if(contradic && !reachall)	//normal contradiction
	  return -2;
  else if(!contradic && !reachall)	//can't reach all pathMark, ignore
	  return -3;

  //left with !contradic && reachall, generate input
...
void PathGuidedSearch::doPathGuided(size_t pos, int depth, SymbolicExecution& prev_ex) {
  SymbolicExecution cur_ex;
  vector<value_t> input;
